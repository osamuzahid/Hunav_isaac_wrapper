#!/usr/bin/env python3
"""Offline smoke: plan CUCR office routes without starting Isaac Sim.

Usage (from wrapper package root or any cwd):
  python3 tools/plan_office_routes.py
"""
...
def main() -> int:
    maps = os.path.join(ROOT, "maps", "office.yaml")
    ok = True
    for scenario in ("office_agents", "office_behaviors"):
        params = _load_scenario(scenario)
        inflation = float(params.get("plan_inflation_radius_m", 0.50))
        ...
        for i, a in enumerate(names):
            for b in names[i + 1 :]:
                ...
                if d < CHOKE_M:
                    ...
                    ok = False
                elif scenario == "office_behaviors":
                    print(f"     {a} vs {b}: min {d:.2f} m")

        if scenario == "office_behaviors":
